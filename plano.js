...
function mostrarImagen(event, numeroPiso) {
    ...
    imagen.onload = function() {
        ...
        personalizarBtn.classList.add("personalizar-btn"); // Agregar la clase al botón de personalización
        personalizarBtn.textContent = "Personalizacion del diseño";
        personalizarBtn.addEventListener("click", function() {
            window.open("https://web.whatsapp.com/send?phone=51978848088&text=Hola+Procero+quiero+personalizar+mi+plano", "_blank");
        });
...
function mostrarImagenEdificio(event, numeroPiso) {
    ...
    const versionEdificio="EG";//planos de edificio en version gratuita
   ...
   const urlEdificio = `https://procero.tech/wp-content/uploads/2023/12/${versionEdificio}_${caracteristicasEdificio}.webp`;
    ...
    imagenEdificio.onerror = function() {
        // imageContainer.innerHTML="";
        const errorContainer = document.createElement("div");
        errorContainer.classList.add("error-container"); // Agregar una clase para el estilo

        const errorMessage = document.createElement("p");
        errorMessage.classList.add("error-message"); // Agregar una clase para el estilo
        errorMessage.textContent = "Solicitar planos de arquitectura con medida ";
